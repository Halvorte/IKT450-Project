...
def training_loop(n_epochs, gru, optimizer, loss_fn, x_train, y_train, x_val, y_val):
    for epoch in range(n_epochs):
        outputs = gru.forward(x_train)  # forward pass
...
def sliding_window(dataframe, window_size):
    ...
    for i in range(len(dataframe) - window_size):
        ...
        # Run training
        training_loop(nr_epochs, gru=gru, optimizer=optimizer, loss_fn=loss_fn, x_train=x_train_tensors_final,
                      y_train=y_train_tensor, x_val=x_val_tensors_final, y_val=y_val_tensor)

        # Prediction
        prediction = gru.forward(x_val_tensors_final)
